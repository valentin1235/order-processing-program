...
static error_t server_on(void)
{
    ...
    if (bind(server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) == -1) {
        printf("* 서버 소켓 정보 바인딩에 실패했습니다 -> %s\n", __func__);
        return ERROR_BIND;
    }

    /* listen via binded socket */
    if (listen(server_socket, 5) == -1) {
        printf("* 서버 listen에 실패했습니다 -> %s\n", __func__);
        return ERROR_LISTEN;
    }

    printf("* 서버가 시작되었습니다. port:3000 -> %s\n", __func__);
    ...
    do {
        ...
        printf("* 클라이언트를 받고있습니다... -> %s\n", __func__);
        client_socket = accept(server_socket, (struct sockaddr*)&client_addr, &client_addr_size);
        g_sock_clients[g_sock_client_count++] = client_socket;
        printf("* 클라이언트를 받았습니다 (소켓 번호 : %d) -> %s\n", client_socket, __func__);
